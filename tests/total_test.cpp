/**
 * @file total_test.cpp
 * @brief EventLoop/Channel tests (gtest)
 */
...
TEST(EventLoopReliability, StopBeforeStartAndRestartWorks) {
    ...
    const bool first_running = wait_until([&] { return loop_ptr->get_run_status(); }, std::chrono::milliseconds(200));
    if (!first_running) {
        loop_ptr->stop();
    }
    ...
    const bool second_running = wait_until([&] { return loop_ptr->get_run_status(); }, std::chrono::milliseconds(200));
    if (!second_running) {
        loop_ptr->stop();
    }
...
TEST(EventLoopReliability, StopWakesUpEpollWait) {
    ...
    const bool running = wait_until([&] { return loop_ptr->get_run_status(); }, std::chrono::milliseconds(200));
    if (!running) {
        loop_ptr->stop();
    }
    EXPECT_TRUE(running);
...
TEST(EventLoopReliability, PipeReadableTriggersCallback) {
    ...
    std::thread t([&] {
        ...
        ctl_ch->set_epollin_callback([&] {
            drain_fd(ctl_read.get());
            loop.stop();
        });

        const bool add_ok = loop.add_fd(read_fd.get(), ch) && loop.add_fd(ctl_read.get(), ctl_ch);
...
TEST(EventLoopReliability, RemoveFdDisablesFurtherCallbacks) {
    ...
    std::thread t([&] {
        ...
        data_ch->set_epollin_callback([&] {
            drain_fd(data_read.get());
            const int c = ++calls;
            if (c == 1) {
                remove_ok.store(loop.remove_fd(data_read.get()));
                bool expected = false;
        ...
        const bool add_ok = loop.add_fd(data_read.get(), data_ch) && loop.add_fd(ctl_read.get(), ctl_ch);
        add_ok_promise.set_value(add_ok);
        ready_promise.set_value();
    ...
    std::thread t([&] {
        ...
        ch1->set_epollin_callback([&] {
            ...
            auto ch2 = std::make_shared<Channel>();
                ch2->set_epollin_callback([&] {
                    drain_fd(data_read.get());
                    ++cb2_calls;
                });

            reset_ok.store(loop.reset_channel(data_read.get(), ch2));
        ...
        const bool add_ok = loop.add_fd(data_read.get(), ch1) && loop.add_fd(ctl_read.get(), ctl_ch);
        add_ok_promise.set_value(add_ok);
        ready_promise.set_value();
...
TEST(EventLoopReliability, SocketpairEcho) {
    ...
    std::thread loop_thread([&] {
        ...
        const bool add_ok = loop.add_fd(server_fd.get(), ch) && loop.add_fd(ctl_read.get(), ctl_ch);
        add_ok_promise.set_value(add_ok);
        ready_promise.set_value();
...
TEST(EventLoopReliability, AddFdRequiresFocusedEvents) {
    ...
    EventLoop loop;
    auto ch = std::make_shared<Channel>();
    EXPECT_FALSE(loop.add_fd(read_fd.get(), ch));
}

TEST(EventLoopReliability, StopIsIdempotentWhenNotRunning) {
    EventLoop loop;
    EXPECT_FALSE(loop.get_run_status());
...
TEST(EventLoopReliability, AddFdNullChannelRejected) {
    ...
    EventLoop loop;
    EXPECT_FALSE(loop.add_fd(read_fd.get(), nullptr));
}
...
TEST(EventLoopReliability, AddResetRemoveMustBeCreatorThread) {
    ...
    std::thread t([&] {
        add_promise.set_value(loop.add_fd(read_fd.get(), ch));
        reset_promise.set_value(loop.reset_channel(read_fd.get(), ch));
        remove_promise.set_value(loop.remove_fd(read_fd.get()));
    });
    t.join();
...
TEST(EventLoopReliability, RemoveUnknownFdRejected) {
    ...
    EventLoop loop;
    EXPECT_FALSE(loop.remove_fd(read_fd.get()));
}
...
TEST(EventLoopReliability, ResetUnknownFdRejected) {
    ...
    auto ch = std::make_shared<Channel>();
    ch->set_epollin_callback([] {});
    EXPECT_FALSE(loop.reset_channel(read_fd.get(), ch));
}
...
TEST(EventLoopReliability, AddSameFdTwiceKeepsOriginalCallback) {
    ...
    auto ch2 = std::make_shared<Channel>();
    ch2->set_epollin_callback([&] { ++cb2_calls; });

    ASSERT_TRUE(loop.add_fd(read_fd.get(), ch1));
    EXPECT_FALSE(loop.add_fd(read_fd.get(), ch2));
...
TEST(EventLoopReliability, ErrorEventsPreferErrorCallback) {
    ...
    ch->set_epollerr_callback([&] {
        ++err_calls;
        loop.stop();
    });

    ASSERT_TRUE(loop.add_fd(server_fd.get(), ch));
...
TEST(EventLoopReliability, RapidStartStopCycles) {
    ...
    for (int i = 0; i < kCycles; ++i) {
        ...
        std::thread t([&] {
            ...
            const bool add_ok = loop.add_fd(ctl_read.get(), ctl_ch);
            ready_promise.set_value();
            if (!add_ok) {
                start_ret_promise.set_value(false);
                return;
        ...
        (void)wait_until([&] { return loop_ptr->get_run_status(); }, std::chrono::milliseconds(200));
        loop_ptr->stop();
...
TEST(EventLoopReliability, ManyFdsAllCallbacksTriggered) {
    ...
    std::thread loop_thread([&] {
        ...
        bool add_ok = loop.add_fd(ctl_read.get(), ctl_ch);

        for (int i = 0; i < kPipes; ++i) {
            auto ch = std::make_shared<Channel>();
            ch->set_epollin_callback([&, i] {
            ...
            add_ok = add_ok && loop.add_fd(read_fds[static_cast<size_t>(i)].get(), ch);
        }
...
TEST(EventLoopReliability, EpollOutTriggersCallback) {
    ...
    ASSERT_TRUE(loop.add_fd(server_fd.get(), ch));
    ASSERT_TRUE(loop.add_fd(ctl_read.get(), ctl_ch));
...
TEST(EventLoopReliability, ResetChannelTogglesFromInToOut) {
    ...
    ch->set_epollin_callback([&] {
        ...
        reset_ok.store(loop.reset_channel(server_fd.get(), ch));
    });
...
TEST(EventLoopReliability, RemoveThenAddSameFdWorks) {
    ...
    ch1->set_epollin_callback([&] {
        ...
        remove_ok.store(loop.remove_fd(read_fd.get()));
        ...
        add_ok.store(loop.add_fd(read_fd.get(), ch2));
...
TEST(EventLoopReliability, MultiWriterStressManyFds) {
    ...
    for (int i = 0; i < kPipes; ++i) {
        ...
        ASSERT_TRUE(loop.add_fd(read_fds[static_cast<size_t>(i)].get(), ch));
    }
